...
import { authors, genres } from "../assets/data/data";
import "../index.css";

const GENRE_ARRAY = Object.values(genres);
GENRE_ARRAY.unshift("All Genres");
const AUTHOR_ARRAY = Object.values(authors);
AUTHOR_ARRAY.unshift("All Authors")

/**
 * The `SearchModal` component is a modal overlay that allows users to search for books based on title,
 * genre, and author.
 *
 * @component
 * @param {Object} props - The properties passed to the component.
 * @param {Function} props.onHide - A function to handle the modal hide event.
 * @returns {React.JSX.Element} The JSX element representing the SearchModal component.
 */
      ...
      >
        <Modal.Body>
          <Form className="overlay__form">
            <Form.Group controlId="search.Title" className="overlay__field">
              <Form.Label className="overlay__label">Title</Form.Label>
              <Form.Control type="text" placeholder="Any" className="overlay__input" />
            </Form.Group>

            <Form.Group controlId="search.Genre" className="overlay__field">
              <div className="overlay__label">Genre</div>
              <select className="overlay__input overlay__input_select">
                {GENRE_ARRAY.map((genre, index) => (
            ...
            <Form.Group controlId="search.Author" className="overlay__field">
              <div className="overlay__label">Author</div>
              <select className="overlay__input overlay__input_select">
              {AUTHOR_ARRAY.map((author, index) => (
        ...
        <Modal.Footer className="overlay__row">
          <Button onClick={onHide} className="overlay__button">Cancel</Button>
          <Button className="overlay__button overlay__button_primary">Search</Button>
        </Modal.Footer>
      </Modal>
    </Container>
  );
...
/**
 * The `ThemeModal` component is a modal overlay that allows the user to select a theme and save their
 * selection.
 *
 * @component
 * @param {Object} props - The properties passed to the component.
 * @param {Function} props.onHide - A function to handle the modal hide event.
 * @returns {React.JSX.Element} The JSX element representing the ThemeModal component.
 */
  ...
  return (
    <Container className="overlay">
      <Modal
        ...
        <Modal.Body>
          <Form className="overlay__form">
            <Form.Group className="overlay__field">
        ...
        <Modal.Footer className="overlay__row">
          <Button onClick={onHide} className="overlay__button">Cancel</Button>
          <Button className="overlay__button overlay__button_primary">Save</Button>
        </Modal.Footer>
      </Modal>
    </Container>
  );
...
/**
 * The `Header` component represents the website's header.
 *
 * @component
 * @returns {React.JSX.Element} The JSX element representing the Header component.
 */
const Header = () => {
  //state for the search modal
  const [searchModal, setSearchModal] = useState(false);
  //state for the theme modal
  const [themeModal, setThemeModal] = useState(false);

   /**
   * Function to handle opening the theme modal.
   */
      ...
      <header className="header">
        <div className="header__inner">
          ...
          <div>
            ...
            {/* theme toggle button */}
            <button className="header__button" onClick={handleOpenTheme}>
              <svg
                className="header__icon"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 960 960"
